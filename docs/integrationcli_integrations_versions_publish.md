...
```
Publishes an integration vesion with the highest snapshot in SNAPHOST state: integrationcli integrations versions publish -n $name --default-token
...
```
      ...
      --config-vars-json string   Json string containing the config variables if both Json string and file is present Json string will only be used.
